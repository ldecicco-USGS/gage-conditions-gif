prep_gage_sites_fun <- function(percentile_color_data_ind, sites_sp, dateTime){
  ...
  if(!file.exists(scipiper::as_data_file(percentile_color_data_ind))){
    scipiper::gd_get(percentile_color_data_ind)
  }
  percentile_color_data <- readRDS(scipiper::as_data_file(percentile_color_data_ind))
